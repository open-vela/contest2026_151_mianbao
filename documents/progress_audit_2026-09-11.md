...
| # | 事项 | 状态 |
|---|------|------|
| 1 | 烧录 DMIC 修复并验证录音 | ⏳ 待做 |
| 2 | 定静音阈值与麦克风增益 | ⏳ 待做（依赖 1 的实测值） |
| 3 | **推送两个未推送的 commit**（`9ead4c4`、`7746f8d`） | ⏳ 代理不通，待网络恢复 |
| 4 | 复核云端响应 8MB 上限 —— 纯静音那轮触发了 `响应超过上限 8388608 字节`，需确认正常录音下不会触发 | ⏳ 待观察 |
| 5 | R3 修正文档（`project_status.md` 描述了不存在的文件） | ☐ 未开始 |
| 6 | R4 重写 README（评委据此复现，**含编译命令定论**） | ☐ 未开始 |
| 7 | R5-R7 清理（模板目录、构建产物） | ☐ 未开始 |

### 11.6 当前环境状态（2026-09-13 晚）

- **Flask**：`192.168.93.141:5000`，已配 MIMO API key，监听 `0.0.0.0`
- **网络方案**：已选定「局域网 + VMware NAT 端口转发」（宿主机热点 `172.20.10.2:5000` → 虚拟机 `192.168.93.141:5000`）
  - 曾卡在 Windows 防火墙，已放行；**建议改回开启防火墙 + 只放行 TCP 5000**
- **板子**：连手机热点（`172.20.10.5`），固件仍是有麦克风问题的旧版
- **代理**：`192.168.1.3:7897` 目前不通，导致 git 推送失败（commit 都在本地）
