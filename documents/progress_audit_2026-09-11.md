...
| 14 | **把「静音门限 900」与「岗位默认值 `AI 应用开发`」合并成一次重烧** | ⏳ 待办（9/18）：两处都只改了代码。门限见 §11.19，岗位默认值见 §11.21 —— **一次烧录把两件一起验掉**（重烧后按 §11.7 的日志自检确认板上是 900） |
...
### 11.19 静音门限 500 → 900（2026-09-14 晚，**未重烧**）
...
> 想从二进制里反查这个常数是**行不通的**：试过在全 `nuttx.bin` 里搜 `mov.w #900` 的机器码，找到的两处都在 `silk_encode_frame_FIX`（OPUS 编解码器）里，是巧合。**真正的确认方式是板上那行日志**（见 §11.7 的自检）。
...
1. **全部未上板**（含 9/14 的门限 900）—— 一次重烧把两件一起验，见 §11.5 第 14 条；
