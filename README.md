...
云端自带一个网页，把面试对话实时显示成聊天气泡 —— 演示时比盯串口方便得多。

| 浏览器在哪 | 打开 |
|---|---|
| 虚拟机桌面 | `http://127.0.0.1:5000/` |
| Windows 宿主机 | `http://127.0.0.1:5000/`（走已有的 5000 端口转发） |
| 热点上的其他设备 | `http://172.20.10.2:5000/` |

> ⚠️ **宿主机浏览器这一路，本队未在演示机上实测过**。现在的转发规则是给板子用的（板子 → `172.20.10.2:5000` → 虚拟机），宿主机自己开 `127.0.0.1:5000` 能不能通要现场确认。**打不开就直接用串口，不影响任何功能。**
