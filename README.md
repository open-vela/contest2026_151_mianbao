...
**多轮**：板子每轮**只带上 `session_id`**，**对话历史完全由云端维护**——端侧请求体里的 `history` 恒为空数组（`app/ai_interview/network_client.c` 内有注释说明）。`session_id` 在第一次请求时由云端生成，之后由端侧持有并回传。会话与历史都只存在于**云端进程内存**中，重启云端即全部清空。
